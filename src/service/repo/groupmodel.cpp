...
void GroupModel::createGroup(::google::protobuf::RpcController* controller,
                   const ::fixbug::CreateGroupRequest* createGroupRequest,
                   ::fixbug::CreateGroupResponse* response,
                   ::google::protobuf::Closure* done) {
    //获取请求数据
    Group group;
    group.setId(createGroupRequest->group().id());
    group.setName(createGroupRequest->group().name());
    group.setDesc(createGroupRequest->group().desc());
    //创建群组
    bool ret = createGroup(group);
    if(ret){
...
void GroupModel::addGroup(::google::protobuf::RpcController* controller,
...
void queryGroups(::google::protobuf::RpcController* controller,
    ...
    for(Group &group : groupVec){   
        ...
        pgroup->set_id(group.getId());
        pgroup->set_name(group.getName());

        pgroup->set_desc(group.getDesc());
    }
    //返回结果
    response->mutable_result()->set_errcode(0);
    response->mutable_result()->set_errmsg("success");
...
void GroupModel::queryGroupUsers(::google::protobuf::RpcController* controller,
    ...
    int groupid = request->groupid();
    //执行查询群组用户业务
    vector<int> vec = queryGroupUSers(userid, groupid);
    //返回结果
    response->mutable_result()->set_errcode(0);
    response->mutable_result()->set_errmsg("success");
    done->Run();
